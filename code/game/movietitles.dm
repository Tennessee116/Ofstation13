	...
	if(mob)
		...
		if(mob.get_preference_value(/datum/client_preference/play_lobby_music) == GLOB.PREF_YES)
			...
			if(GLOB.end_credits_song == null)
				sound_to(mob, sound('resources/sound/music/THUNDERDOME.ogg', wait = 0, volume = 40, channel = GLOB.lobby_sound_channel))
...
/proc/generate_titles()
	...
	if(!GLOB.end_credits_title)
		...
		possible_titles += "THE CREW LEARNS ABOUT [pick("LOVE", "DRUGS", "THE DANGERS OF MONEY LAUNDERING", "XENIC SENSITIVITY", "INVESTMENT FRAUD", "KELOTANE ABUSE", "RADIATION PROTECTION", "SACRED GEOMETRY", "STRING THEORY", "ABSTRACT MATHEMATICS", "[pick("UNATHI", "SKRELLIAN", "VOX")] MATING RITUALS", "ANCIENT CHINESE MEDICINE")]"
		possible_titles += "A VERY [pick("NANOTRASEN", "EXPEDITIONARY", "PHORON", "MARTIAN")] CHRISTMAS"
